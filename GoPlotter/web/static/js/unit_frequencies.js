...
function setupTabs() {
    document.querySelectorAll('.page-tab').forEach(tab => {
        tab.addEventListener('click', function () {
            ...
            else if (name === 'pending-review') loadPendingRequests();
...
function setupEventListeners() {
    document.getElementById('refreshBtn')?.addEventListener('click', () => {
        ...
        else if (active === 'drafts')        loadDrafts();
        else if (active === 'pending-review') loadPendingRequests();
    ...
    document.getElementById('requestSearch')?.addEventListener('input', e => {
        renderRequests(filterRequests(myRequests, e.target.value, document.getElementById('requestStatusFilter')?.value));
    });
    document.getElementById('requestStatusFilter')?.addEventListener('change', e => {
        renderRequests(filterRequests(myRequests, document.getElementById('requestSearch')?.value || '', e.target.value));
    });
    ...
    document.getElementById('pendingSearch')?.addEventListener('input', e => {
        ...
        renderPendingRequests(filterPendingRequests(pendingRequests, e.target.value, purpose));
    });
    document.getElementById('pendingPurposeFilter')?.addEventListener('change', e => {
        ...
        renderPendingRequests(filterPendingRequests(pendingRequests, search, e.target.value));
    });
...
async function loadAssignments() {
    showContainerLoading('permanentContainer', 'Loading permanent assignments...');
    showContainerLoading('temporaryContainer', 'Loading temporary assignments...');
    try {
        const res  = await fetch('/api/frequency/units');
        const data = await res.json();
        if (data.units) {
            unitsData = data.units;
            populateUnitFilters(data.units);
            updateStats(data.units);
            renderAssignmentTab('permanent');
            renderAssignmentTab('temporary');
        } else {
            showContainerError('permanentContainer', 'Failed to load assignments');
            showContainerError('temporaryContainer', 'Failed to load assignments');
        }
    } catch (err) {
        showContainerError('permanentContainer', 'Error: ' + err.message);
        showContainerError('temporaryContainer', 'Error: ' + err.message);
    }
}
...
function renderAssignmentTab(bucket) {
    ...
    const containerId = isPerm ? 'permanentContainer' : 'temporaryContainer';
    const now      = new Date();
...
async function loadMyRequests() {
    ...
    try {
        ...
        document.getElementById('tabBadgeRequests').textContent = myRequests.length;
        renderRequests(myRequests);
    } catch (err) {
...
function viewRequest(id) {
    ...
    let r = myRequests.find(x => x.request?.id === id) ||
            pendingRequests.find(x => x.request?.id === id);
    if (!r) {
        for (const ud of unitsData) {
    ...
    let actions = '<button class="btn" onclick="document.getElementById(\'requestModal\').style.display=\'none\'">Close</button>';
...
async function loadPendingRequests() {
    ...
    try {
        ...
        document.getElementById('tabBadgePending').textContent = pendingRequests.length;
...
function openApprovalModal() {
    document.getElementById('requestModal').style.display = 'none';
    const r = pendingRequests.find(x => x.request?.id === currentRequestId) ||
              myRequests.find(x => x.request?.id === currentRequestId);
    if (r) {
        const req = r.request;
...
async function submitApproval() {
    const reqId = document.getElementById('approvalRequestId').value;
    const body  = {
...
function loadDrafts() {
    ...
    if (!draft) {
        container.innerHTML = `<div class="alert alert-warning" style="margin:20px">Draft data is corrupted. <button class="btn btn-sm" onclick="discardDraft()">Discard</button></div>`;
        return;
    }
