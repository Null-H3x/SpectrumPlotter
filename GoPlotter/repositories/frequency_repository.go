...
func (r *FrequencyRepository) CreateFrequencyRequest(request *models.FrequencyRequest) error {
	query := `
		INSERT INTO frequency_requests (
			...
			coverage_area, operating_area_geojson, authorized_radius_km, start_date, end_date, hours_of_operation,
	...
	return r.db.QueryRowx(query,
		...
		request.CoverageArea, request.OperatingAreaGeoJSON, request.AuthorizedRadiusKm, request.StartDate, request.EndDate, request.HoursOfOperation,
...
func (r *FrequencyRepository) ResubmitFrequencyRequest(id uuid.UUID, requestedBy uuid.UUID, input models.CreateFrequencyRequestInput) (*models.FrequencyRequest, error) {
	...
	err := r.db.Get(&req, `
		UPDATE frequency_requests SET
			...
			coverage_area         = NULLIF($18, ''),
			authorized_radius_km  = $19,
			start_date            = $20,
			end_date              = $21,
			hours_of_operation    = NULLIF($22, ''),
			num_transmitters      = $23,
			num_receivers         = $24,
			is_encrypted          = $25,
			encryption_type       = NULLIF($26, ''),
			classification        = $27,
			requires_coordination = $28,
			coordination_notes    = NULLIF($29, ''),
			justification         = $30,
			mission_impact        = NULLIF($31, ''),
			status                = 'pending',
			denied_reason         = NULL,
			review_notes          = NULL,
			reviewed_by           = NULL,
			reviewed_at           = NULL,
			updated_at            = NOW()
		WHERE id = $32 AND requested_by = $33 AND status = 'denied'
		RETURNING *`,
		input.RequestType,
		input.Priority,
